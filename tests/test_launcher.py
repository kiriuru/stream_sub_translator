...
class LauncherTests(unittest.TestCase):
    ...
    def test_apply_startup_mode_to_config_sets_browser_profile_lock_for_quick_start(self) -> None:
        launcher = self._launcher()
    ...
    def test_apply_startup_mode_to_config_clears_browser_profile_lock_for_local_ai(self) -> None:
        launcher = self._launcher()
        ...
        launcher._apply_startup_mode_to_config(launcher_module.STARTUP_MODE_LOCAL, launcher_module.LAUNCH_OPTION_CPU)
    ...
    def test_build_splash_html_includes_locale_switcher_and_compact_log(self) -> None:
        html = launcher_module._build_splash_html("Stream Subtitle Translator", locale="ru")
        self.assertIn('data-locale="ru"', html)
        self.assertIn('data-locale="en"', html)
        self.assertIn("set_ui_language", html)
        self.assertIn("max-height: 112px", html)
        self.assertIn("launcher.profile.quick_start", html)

    def test_build_splash_html_web_only_hides_profile_panel(self) -> None:
        html = launcher_module._build_splash_html(
            "Stream Subtitle Translator",
            locale="en",
            web_speech_only=True,
        )
        self.assertIn('class="web-speech-only"', html)
        self.assertIn("launcher.status.web_only_initial", html)
        self.assertIn("body.web-speech-only .profile-panel", html)

    def test_launch_context_exposes_web_speech_only_flag(self) -> None:
        launcher = self._launcher(web_speech_only=True)
        self.assertTrue(launcher._context.web_speech_only)
        self.assertFalse(self._launcher()._context.web_speech_only)
    ...
    def test_web_speech_only_skips_launch_option_wait(self) -> None:
        ...
        self.assertEqual(remote_role, "disabled")
        self.assertFalse(allow_lan)

    def test_save_ui_language_persists_config(self) -> None:
        config_path = self.paths.data_dir / "config.json"
        launcher_module._save_ui_language(config_path, "ru")
        payload = json.loads(config_path.read_text(encoding="utf-8"))
        self.assertEqual(payload["ui"]["language"], "ru")
        self.assertEqual(launcher_module._load_ui_language(config_path), "ru")

    def test_load_ui_layout_reads_compact(self) -> None:
        config_path = self.paths.data_dir / "config.json"
        config_path.write_text(json.dumps({"ui": {"layout": "compact"}}), encoding="utf-8")
        self.assertEqual(launcher_module._load_ui_layout(config_path), launcher_module.UI_LAYOUT_COMPACT)

    def test_desktop_api_set_ui_language_updates_splash(self) -> None:
        config_path = self.paths.data_dir / "config.json"
        window = _FakeWindow()
        api = launcher_module.DesktopApi(
            lambda: SimpleNamespace(data_dir=str(self.paths.data_dir)),
            external_url_opener=lambda _url: False,
            launch_mode_selector=lambda _selection: False,
            window_getter=lambda: window,
        )
        saved = api.set_ui_language("ru")
        self.assertEqual(saved, "ru")
        self.assertEqual(launcher_module._load_ui_language(config_path), "ru")
        self.assertTrue(any("__sstApplySplashLocale" in call for call in window.js_calls))

    def test_resize_window_for_dashboard_uses_compact_sizes(self) -> None:
        launcher = self._launcher()
        window = SimpleNamespace(resize_calls=[], min_size=None)

        def resize(width: int, height: int) -> None:
            window.resize_calls.append((width, height))

        window.resize = resize
        config_path = self.paths.data_dir / "config.json"
        config_path.write_text(json.dumps({"ui": {"layout": "compact"}}), encoding="utf-8")
        layout = launcher._resize_window_for_dashboard(window)
        self.assertEqual(layout, launcher_module.UI_LAYOUT_COMPACT)
        sizes = launcher_module.DASHBOARD_WINDOW_SIZES[launcher_module.UI_LAYOUT_COMPACT]
        self.assertEqual(window.resize_calls[-1], (sizes["width"], sizes["height"]))

    def test_resize_pywebview_window_sets_min_size_before_resize(self) -> None:
        class _OrderWindow:
            def __init__(self) -> None:
                self.order: list[str] = []

            def resize(self, _width: int, _height: int) -> None:
                self.order.append("resize")
            ...
            def min_size(self, value: tuple[int, int]) -> None:
                self.order.append("min_size")
                self._min_size = value

        window = _OrderWindow()
        launcher_module._resize_pywebview_window(
            window,
            width=400,
            height=844,
            min_width=360,
        ...
        self.assertEqual(window.order, ["min_size", "resize"])
