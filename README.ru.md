...
Этот README описывает текущий desktop product surface для линии `0.3.1`.
...
- Delta notes для `0.3.1`: [docs/DESKTOP_RELEASE_CHANGELOG_0.3.1.md](./docs/DESKTOP_RELEASE_CHANGELOG_0.3.1.md)
- Текущие branch follow-up notes (после `0.3.1`): `docs/CHANGELOG.md` -> `Unreleased`

## Ключевое про 0.3.1

`0.3.1` — релиз стабилизации поверх `0.3.0`. Публичные контракты (`/api`, WebSocket payloads, формы overlay/runtime/transcript) не меняются. Базовый local-first продукт сохранён.

В него вошли:

- `backend/versioning.py::PROJECT_VERSION = "0.3.1"` — single source of truth.
- `RuntimeOrchestrator` стал фасадом над явными контроллерами в `backend/core/runtime/` (state, lifecycle, metrics, session, segments, browser-worker bookkeeping, speech sources, audio capture, processing tasks, translation runtime, transcript pipeline, output fanout).
- `SubtitleRouter` разделён на `subtitle_lifecycle_core.py` (FSM, TTL, релевантность), `subtitle_presentation.py` (сборка payload, слоты стилей, слияние partial и финала) и тонкий публикующий фасад.
- Провайдеры перевода вынесены в отдельный пакет `backend/translation/providers/`; кеш перевода (`backend/core/cache_manager.py`) — in-memory LRU c дебаунс-персистом и карантином повреждённого файла кеша.
- Запись config/profiles стала Windows-safe атомарной (`backend/core/atomic_io.py`, `os.replace()`); повреждённый `user-data/config.json` уезжает в `*.corrupt-<timestamp>.json`, и приложение поднимается на дефолтах.
- Маршрутизация перевода — через стабильные карточки слотов `translation_1 .. translation_5` с выбором провайдера на каждую линию; dashboard предупреждает о незаполненных обязательных настройках.
- Удалена ветка Microsoft Edge: Web Speech всегда открывается в отдельном окне **Google Chrome** с изолированным `user-data-dir`, `HIGH_PRIORITY_CLASS`, opt-out из EcoQoS на Windows 10/11, Screen Wake Lock, терминальной деградацией `recognition_network_unreachable` и новым health-сигналом `voice_below_recognition_threshold`.
- `/google-asr-edge` и `/google-asr-experimental-edge` возвращают `404` (зафиксировано регрессией `tests/test_api_and_websockets.py`).
- Live проверка обновлений: `POST /api/updates/check` опрашивает GitHub Releases (опт-ин через `updates.enabled`), сохраняет `updates.latest_known_version` + `updates.last_checked_utc`; bootstrap-лаунчер тихо показывает диалог только при наличии новой версии.
- OpenAI helper endpoints (`GET /api/openai/recommended-models`, `POST /api/openai/models`, `POST /api/openai/usable-models`) позволяют заполнить поле `model` без хранения ключей во фронте.
- Пользовательские логи — в корневом `logs/` (legacy `user-data/logs/` мигрирует при старте); локальные модели ASR — в `user-data/models/`.
- Встроенные эффекты появления субтитров: `none`, `fade`, `subtle_pop`, `slide_up`, `zoom_in`, `blur_in`, `glow` — в превью дашборда и в OBS overlay.
- В дашборд добавлена вкладка `Help / Помощь` после `Tools & Data` (topic-tabs: обзор, распознавание/тюнинг, перевод, субтитры/стиль, OBS, инструменты/диагностика, desktop/remote).

При этом не менялись базовые product constraints:

- local-first остаётся default;
- UI по-прежнему plain HTML/CSS/JS через FastAPI;
- `browser_google` и `browser_google_experimental` не удалены;
- локальный Parakeet path остаётся доступным;
- remote mode остаётся отдельным explicit LAN-сценарием и заморожен.
...
5. В стартовом окне (launcher) выберите профиль запуска:
   - `Quick Start (Web Speech)` (название в UI оставлено на английском)
   - `NVIDIA GPU (CUDA)`
   - `CPU-only`
   - `Remote Controller`
   - `Remote Worker`
...
В `0.3.1` сохранена и уточнена явная архитектура из `0.3.0`.
...
Внешний вид dashboard в `0.3.1` не переработан; основные изменения остаются на уровне внутренней архитектуры и устойчивости runtime, унаследованных из `0.3.0`.
...
- Desktop behavior зафиксирован:
  ...
  - Ветка Microsoft Edge удалена; `/google-asr-edge` и `/google-asr-experimental-edge` возвращают `404`.
...
Classic Web Speech в `0.3.1` включает:
...
`0.3.1` сохраняет явный config contract из `0.3.0` и усиливает его:

- config versioned и проходит через явные migrations (`backend/core/config_migrations.py`, текущая `CURRENT_CONFIG_VERSION = 6`);
- config normalization живёт в `backend/config/` (`defaults.py`, `secrets.py`, `normalizers/asr.py|browser.py|obs.py|remote.py|subtitles.py|translation.py`);
...
- `backend/versioning.py` (`PROJECT_VERSION = "0.3.1"`) — single source of truth для версии приложения.
...
- structured runtime events:
  - смотрите `logs/runtime-events.jsonl`
...
Для текущего состояния `0.3.1` были прогнаны:
