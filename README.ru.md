...
Этот README описывает текущий desktop product surface для линии `0.4.3`.
...
В GitHub Releases — готовые bootstrap **exe** (собираются локально из `desktop/` и `.spec` в репозитории + локальных `build-*.bat` / `publish-desktop-releases*.ps1`, не в git):

- `Stream Subtitle Translator.exe` — стандартный bootstrap (payload следует `PROJECT_VERSION`, в этом дереве **0.4.3**)
...
Сборка desktop exe (локальные скрипты — не в публичном git):
...
- `backend/versioning.py` (`PROJECT_VERSION = "0.4.3"`) — single source of truth для версии приложения.
...
**Клон GitHub** содержит backend, frontend, overlay, `tests/`, **`desktop/`** и PyInstaller `*.spec`.

- Запуск и разработка без exe: `start.bat`.
- Сборка bootstrap exe (Windows + venv с `requirements.desktop.txt`) — скрипты **`build-*.bat`** и **`publish-desktop-releases*.ps1`** держите локально (не в git; см. `.gitignore`). Типичный порядок — [docs/TECHNICAL_ARCHITECTURE.md](./docs/TECHNICAL_ARCHITECTURE.md) §14 и §20.

В git **не** коммитятся артефакты (`dist/`, `build/`, собранные exe) и сами build/publish-скрипты.
...
- версионный bundle (локально): `dist\desktop-releases\v0.4.3\` (`01-bootstrap-onefile\`, …) при publish этой линии; в старых деревьях может оставаться `v0.4.1\` или `v0.4.0\`.
...
Для `0.4.3`: после правок запускай `python -m unittest discover -s tests` (включая desktop/bootstrap tests в `tests/`). Сборка bootstrap exe — локально на Windows; см. [docs/TECHNICAL_ARCHITECTURE.md](./docs/TECHNICAL_ARCHITECTURE.md) §20.
