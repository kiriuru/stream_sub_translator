...
4. В splash-окне выберите профиль запуска.
5. Дождитесь открытия dashboard.

## Профили запуска
- `Quick Start`
  Браузерный путь распознавания для максимально быстрого старта.
- `NVIDIA GPU (CUDA)`
  Локальный AI-путь распознавания с приоритетом GPU.
- `CPU-only`
  Локальный AI-путь распознавания без GPU-ускорения.
