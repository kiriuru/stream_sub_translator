...
Этот README описывает текущий desktop product surface для линии `0.3.2`.
...
- Delta notes для `0.3.2`: [docs/DESKTOP_RELEASE_CHANGELOG_0.3.2.md](./docs/DESKTOP_RELEASE_CHANGELOG_0.3.2.md)
- Предыдущий delta (`0.3.1`): [docs/DESKTOP_RELEASE_CHANGELOG_0.3.1.md](./docs/DESKTOP_RELEASE_CHANGELOG_0.3.1.md)
- Текущие branch follow-up notes (после `0.3.2`): `docs/CHANGELOG.md` → `Unreleased`

## Ключевое про 0.3.2

`0.3.2` добавляет опциональную **замену слов после ASR** (до перевода и вывода), поднимает **`config_version` до 7** и секцию `source_text_replacement`, продолжает укреплять **Web Speech** (`browser-web-speech-recognition-policy.js`, доработки `browser-asr-session-manager.js`), вводит новые **пресеты стилей** субтитров и существенно расширяет **`docs/TECHNICAL_ARCHITECTURE.md`** (в том числе глава про локальный **NVIDIA Parakeet**: захват, RNNoise, VAD, очередь сегментов, quality vs low-latency). Публичные `/api`-маршруты не меняются; старые `config.json` дополняются при загрузке.

База стабилизации **`0.3.1`** поверх **`0.3.0`** сохраняется (модульный `RuntimeOrchestrator`, split `SubtitleRouter`, `backend/translation/providers`, атомарный config, слоты перевода, окно Chrome worker, проверка обновлений, Help, эффекты субтитров, логи в `logs/`). Подробности — в delta `0.3.1`.

В релиз `0.3.2` дополнительно входят:

- `backend/versioning.py::PROJECT_VERSION = "0.3.2"` — single source of truth.
- Пост-ASR слой: `backend/core/source_text_replacement.py`, `backend/data/source_text_builtin_pairs.json`, UI **Инструменты и данные** (`source-text-replacement-panel.js`: поля слово/замена, список с чекбоксами и одна кнопка удаления выбранных, глобальное сохранение для применения к рантайму), `TranscriptController` с `config_getter`.
- **Web Speech:** политика overlap и улучшения session manager (см. changelog).
- **Стили:** пресеты `accessibility_high_contrast`, `dark_cinema`, `meeting_soft`.
- **Тесты:** полный прогон `298` тестов, `OK` (включая `tests/test_source_text_replacement.py`).
...
- `backend/versioning.py` (`PROJECT_VERSION = "0.3.2"`) — single source of truth для версии приложения.
...
Для текущего состояния `0.3.2` были прогнаны:
