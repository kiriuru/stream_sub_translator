...
Этот README описывает текущий desktop product surface для линии `0.4.0`.
...
- Полный технический документ: [docs/TECHNICAL_ARCHITECTURE.md](./docs/TECHNICAL_ARCHITECTURE.md) (observability browser ASR §11.4, WebSocket/replay §9, preview supersession §12.2)
- Единый changelog: [docs/CHANGELOG.md](./docs/CHANGELOG.md)
- Delta notes для `0.4.0`: [docs/DESKTOP_RELEASE_CHANGELOG_0.4.0.md](./docs/DESKTOP_RELEASE_CHANGELOG_0.4.0.md)
- Предыдущий delta (`0.3.2`): [docs/DESKTOP_RELEASE_CHANGELOG_0.3.2.md](./docs/DESKTOP_RELEASE_CHANGELOG_0.3.2.md)
- Полная история: [docs/CHANGELOG.md](./docs/CHANGELOG.md)

## Ключевое про 0.4.0

`0.4.0` объединяет **наблюдаемость и устойчивость Browser Speech** на бэкенде и **desktop packaging/UX**: опциональный **`Stream Subtitle Translator Only Web.exe`**, **блокировку Local Parakeet** при Web Speech quick start (`asr.desktop_profile_lock`) и **неблокирующий старт dashboard**. Публичные `/api`-маршруты и инварианты субтитров не меняются; `config_version` — **7**.

База **`0.3.2`** сохраняется — [docs/DESKTOP_RELEASE_CHANGELOG_0.3.2.md](./docs/DESKTOP_RELEASE_CHANGELOG_0.3.2.md).

- `PROJECT_VERSION = "0.4.0"` — single source of truth.
- Backend: `browser_asr_*`, WebSocket queues, preview supersession, fix `browser_asr_worker_connected()`.
- Desktop: Only Web exe, profile lock, `desktop-profile-lock.js`, `main.js` / `desktop.js`.
- **Тесты:** **336** тестов, `OK` (`test_browser_asr_observability`, `test_desktop_profile_lock`, …).
...
Desktop release поставляется одним или двумя exe в одной папке релиза:

- `Stream Subtitle Translator.exe` — полный splash с профилями
- `Stream Subtitle Translator Only Web.exe` — только Web Speech (опциональный второй артефакт)
...
5. В стартовом окне выберите профиль:
   ...
   - `Remote Controller`
   - `Remote Worker`
6. Dashboard открывается сразу; настройки догружаются в фоне.
...
- Стандарт: `build-bootstrap-launcher.bat` → `dist\bootstrap-launcher\Stream Subtitle Translator.exe`
- Only Web: `build-bootstrap-launcher-web-only.bat` → `dist\bootstrap-launcher-web-only\Stream Subtitle Translator Only Web.exe`
- Публикация: `publish-desktop-releases.ps1` и `publish-desktop-releases-web-only.ps1`
...
Внешний вид dashboard в линии `0.3.x` не переработан; основные изменения остаются на уровне внутренней архитектуры и устойчивости runtime, унаследованных из `0.3.0`.
...
- После **Quick Start (Web Speech)** или **Only Web**: `asr.desktop_profile_lock = browser_speech` — в Recognition нет Local Parakeet, save/load держит browser mode, пока не запустите с **NVIDIA GPU** или **CPU-only**. Код: `frontend/js/dashboard/desktop-profile-lock.js`, `desktop/launcher.py`, `backend/config/normalizers/asr.py`, `AsrConfig.desktop_profile_lock`.
...
- `backend/versioning.py` (`PROJECT_VERSION = "0.4.0"`) — single source of truth для версии приложения.
...
- Соберите bootstrap: `build-bootstrap-launcher.bat` и при необходимости `build-bootstrap-launcher-web-only.bat`.
- Опубликуйте в release folders: `publish-desktop-releases.ps1` и `publish-desktop-releases-web-only.ps1`.
...
Верификация `0.4.0`: полный discover — **336** тестов, `OK` (см. [docs/CHANGELOG.md](./docs/CHANGELOG.md)).

Сборка релиза:

- `cmd /c build-bootstrap-launcher.bat`
- `cmd /c build-bootstrap-launcher-web-only.bat`
- `powershell -NoProfile -ExecutionPolicy Bypass -File .\publish-desktop-releases.ps1`
- `powershell -NoProfile -ExecutionPolicy Bypass -File .\publish-desktop-releases-web-only.ps1`

Артефакты в release folders:

- `Stream Subtitle Translator.exe`
- `Stream Subtitle Translator Only Web.exe`
