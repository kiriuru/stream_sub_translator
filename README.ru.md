...
- translation routing теперь управляется через стабильные slot card-и `translation_1 .. translation_5` с per-line provider selection;
- provider settings editor умеет следовать за выбранным translation slot и предупреждает, если у включённых линий не хватает обязательных credentials;
...
Если в старой установке ещё осталась корневая папка `logs/`, launcher/runtime перенесёт эти файлы в `user-data/logs/`.
...
Legacy desktop installs с корневой `logs/` автоматически мигрируют эти файлы в `user-data/logs/` при старте launcher/runtime.
