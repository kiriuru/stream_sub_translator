...
## Что изменилось в 0.2.9.2
- Исправлено сохранение языка интерфейса. Теперь язык проходит через сохранённый desktop config, а не живёт только в browser storage.
- Добавлено более широкое regression-покрытие на save/load основных групп настроек.
- Исправлена карточка `Переведённый результат`: после завершения нормального translation update она больше не должна ложно писать, что перевод выключен.
- Версия проекта теперь последовательно используется в формате `0.2.9.2` в runtime, API и документации.
