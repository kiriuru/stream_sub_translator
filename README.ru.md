...
## Что изменилось в 2.8.3
- Интегрировано исправленное поведение browser worker из рабочей desktop-сборки.
- Browser worker больше не перезаписывает `continuous_results` после загрузки настроек.
- Подавлены быстрые дубли одинаковых overlay payload, из-за которых переводы повторно отправлялись в overlay.
