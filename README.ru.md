...
- Полный changelog для `0.2.9.0`: [docs/DESKTOP_RELEASE_CHANGELOG_0.2.9.0.md](./docs/DESKTOP_RELEASE_CHANGELOG_0.2.9.0.md)
- Delta changelog для `0.2.9.1`: [docs/DESKTOP_RELEASE_CHANGELOG_0.2.9.1.md](./docs/DESKTOP_RELEASE_CHANGELOG_0.2.9.1.md)
...
2. Проверьте, что присутствует `Stream Subtitle Translator.exe`.
3. Запустите `Stream Subtitle Translator.exe`.
4. Дождитесь, пока bootstrap launcher при первом запуске сам разложит managed runtime рядом.
5. В splash-окне выберите один из профилей запуска:
...
### Browser Speech
- Работает через отдельное выделенное окно Chrome/Chromium/Edge (`/google-asr`).
- Поведение desktop-версии здесь зафиксировано жёстко:
  - SST всегда открывает Browser Speech как отдельное окно браузера с адресной строкой.
  - Launcher использует для этого окна отдельный isolated browser profile.
  - Никакого переключателя режима окна browser worker в desktop UI больше нет. Режим с адресной строкой является единственным поддерживаемым поведением.
  - Это поведение нельзя снова менять на `--app`, popup-launcher страницы, скрытые bootstrap-окна или открытие внутри обычной вкладки.
...
## Что изменилось в 0.2.9.1
- Для desktop Browser Speech снова жёстко восстановлено старое поведение: отдельное окно Chrome/Chromium/Edge с видимой адресной строкой и isolated worker profile.
- Переключатель режима окна browser worker убран из desktop UI. Режим с адресной строкой теперь является единственным поддерживаемым поведением.
- Clean portable AI bootstrap теперь заранее seed'ит офлайн `lightning 2.4.0` перед установкой NeMo ASR зависимостей, чтобы fresh install больше не падал на missing `lightning`.
- Версия проекта теперь последовательно используется в формате `0.2.9.1` в runtime, API и документации.
