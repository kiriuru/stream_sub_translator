...
Подробный список изменений — [docs/DESKTOP_RELEASE_CHANGELOG_0.4.0.md](./docs/DESKTOP_RELEASE_CHANGELOG_0.4.0.md) (формат как [v0.2.9.2](https://github.com/kiriuru/stream_sub_translator/releases/tag/v0.2.9.2)): компактный дашборд и ресайз окна, observability Browser ASR, lock Parakeet после Web Speech quick start, быстрый старт dashboard (панели сразу; лаунчер переходит на дашборд без ~20 с зависания на pywebviewready), второй установщик **Only Web.exe**. `PROJECT_VERSION = "0.4.0"`; `config_version` — **7**; публичные `/api` и контракт субтитров без изменений.

Предыдущий релиз: [docs/DESKTOP_RELEASE_CHANGELOG_0.3.2.md](./docs/DESKTOP_RELEASE_CHANGELOG_0.3.2.md). История: [docs/CHANGELOG.md](./docs/CHANGELOG.md).
...
- `Stream Subtitle Translator.exe` — стандартный bootstrap 0.4.0
- `Stream Subtitle Translator Only Web.exe` — **новый в 0.4.0**, только Web Speech
...
Профили на splash без изменений относительно прошлых релизов. **Новое в 0.4.0:** Quick Start и **Only Web** выставляют `asr.desktop_profile_lock` — [delta 0.4.0](./docs/DESKTOP_RELEASE_CHANGELOG_0.4.0.md).
...
- версионный bundle (локально): `dist\desktop-releases\v0.4.0\` (`01-bootstrap-onefile\`, `01-bootstrap-web-only-onefile\`, `02-managed-app-onefolder\`, `03-installers-both\`, `README.txt`)
...
`0.4.0`: **336** тестов `OK` в полном локальном дереве; в публичном репозитории — tracked subset (включая `test_browser_asr_observability`). Сборка bootstrap — только локально, см. [docs/TECHNICAL_ARCHITECTURE.md](./docs/TECHNICAL_ARCHITECTURE.md) §20.
