...
В GitHub Releases — готовые bootstrap **exe** (собираются локально из исходников в репозитории, включая `desktop/` и PyInstaller-скрипты):
...
**Клон GitHub** содержит backend, frontend, overlay, `tests/`, **`desktop/`**, `build-*.bat`, `publish-desktop-releases*.ps1` и `*.spec`.

- Запуск и разработка без exe: `start.bat`.
- Сборка bootstrap exe (Windows + venv с `requirements.desktop.txt`):

- `build-desktop.bat`, `build-bootstrap-launcher.bat`, `build-bootstrap-launcher-web-only.bat`
- `publish-desktop-releases.ps1`, `publish-desktop-releases-web-only.ps1`

В git **не** коммитятся артефакты (`dist/`, `build/`, собранные exe) — см. `.gitignore`.
