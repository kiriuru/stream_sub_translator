...
- Полный технический документ: [docs/TECHNICAL_ARCHITECTURE.md](./docs/TECHNICAL_ARCHITECTURE.md) (фасад RuntimeOrchestrator §6, mutex WebSocket §9, transport browser worker §9.1, стабильность UI §16.6, пайплайн Parakeet §17, desktop §14, тесты §20)
...
**`0.4.2`** — полная дельта в [docs/CHANGELOG.md](./docs/CHANGELOG.md#042):

- Кеш SHA-256 целостности модели Parakeet (один хеш на жизнь процесса вместо хеширования на каждый `/api/runtime/status` / `/api/health`, лечит idle latency 3–10 с на свежих установках с sha256 в `manifest.json`).
- Drop-in замена exe теперь очищает устаревшее содержимое `app-runtime/` перед извлечением нового payload — managed runtime после апдейта всегда чистый.
- Вендорное колесо `antlr4-python3-runtime==4.9.3` (`vendor/python-wheels/`) + `backend/bootstrap_pip_pins.py` убирают flaky sdist-сборку NeMo на Windows.
- Эффекты субтитров теперь анимируют **только свежие фрагменты** partial-обновления (ранее набранная часть остаётся статичной); shape-signature fast path сохраняет wrapper/stage/row/surface DOM между partial-кадрами; переход transient→completed доконсолидируется in place; slow path переиспользует уже завершённый source-блок при появлении строки перевода.
- Тематические пресеты субтитров переработаны (10 различимых тем: anime stream, fallout terminal, retro terminal, cyberpunk neon, noir caption, comic burst, cinema plate, max contrast, …) с кириллически-безопасными font fallback chains.
- 28 popular Google Fonts включены прямо в `fonts/` (загрузка шрифтов на первом запуске больше не нужна); `system`-записи в font catalog теперь сохраняются при save/import.
- Style editor: dropdown шрифта показывает реально выбранное семейство активного пресета; добавлен selector «Apply preset» для применения base-стиля пресета только к конкретному line slot override.
- Browser Speech worker заново загружает `/api/settings/load` перед сохранением — кнопка «Сохранить» больше не затирает правки, сделанные в дашборде между открытием окна и нажатием save.
- Compact-layout audit: технические панели (Recognition / Tuning / ASR Advanced) сохраняют hints, eyebrow-метки и inline-notes в compact-режиме; обрезается только декоративный текст в нетехнических вкладках.
- Parakeet tuning controls (`latency preset`, `streaming_decode`, `partial_emit_mode`, `partial_min_new_words`) теперь видны независимо от текущего `asr.mode` — скрываются только при `desktop_profile_lock="browser_speech"` (Web-Speech-only install).
- Opt-in deep-diagnostics: JSONL-traces (`api-trace.jsonl`, `pipeline-trace.jsonl`, `ui-trace.jsonl`, `startup-journey.jsonl`) и события `runtime_lifecycle.*` по умолчанию выключены, поднимаются через `SST_DEEP_DIAGNOSTICS=1` или per-channel `SST_TRACE_*=1`. См. [docs/ETALON_RUNTIME_VERIFICATION.md](./docs/ETALON_RUNTIME_VERIFICATION.md#31-enabling-deep-traces).
- Тонкий фасад `RuntimeOrchestrator` + mixins, сериализация send WebSocket/browser ASR, hardening store/панелей (без сброса caret/focus), ротация desktop-логов, расширенный набор регресс-тестов.

`PROJECT_VERSION = "0.4.2"`; `config_version` — **7**; публичные `/api` и контракт субтитров без изменений.

**`0.4.1`** — [docs/DESKTOP_RELEASE_CHANGELOG_0.4.1.md](./docs/DESKTOP_RELEASE_CHANGELOG_0.4.1.md): streaming Parakeet + word-growth partials, `LocalAsrPipeline`, пресеты Tuning и согласованные слайдеры, runtime-строка, поля Tools (`streaming_decode`, emit mode, min words), один low-latency провайдер в UI, расширенный `AsrDiagnostics`. `PROJECT_VERSION = "0.4.1"`; `config_version` — **7**; публичные `/api` и контракт субтитров без изменений.

**`0.4.0` и ранее** — [docs/CHANGELOG.md](./docs/CHANGELOG.md) (компактный дашборд, Browser ASR observability, **Only Web.exe**, desktop profile lock и предыдущие линии).

Базовые ограничения продукта не менялись:
...
Сборка desktop exe (только локальное dev-дерево, **не** в публичном GitHub):
...
Разработка из клона GitHub: **`start.bat`** (backend + frontend, без каталога `desktop/`).
