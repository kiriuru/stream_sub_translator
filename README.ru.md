...
Этот README описывает desktop release surface для `0.3.0`.
...
- experimental backend provider `google_legacy_http_experimental`.
...
- `backend/core/` для runtime, bootstrap, WS, config, logging, providers и shared utilities;
...
- Управление списком и порядком target languages.
- Просмотр последних результатов перевода.
- Translation pipeline теперь отделён от live source-final path: source final публикуется сразу, а fan-out по target languages идёт асинхронно и stale jobs подавляются, если уже потеряли lifecycle relevance.
...
- Выбирается через `Recognition -> Local ASR provider`, при этом `Recognition method` остаётся `Local Parakeet`.
- Использует backend audio capture path и отправляет `PCM16 mono 16 kHz` напрямую из backend в experimental legacy Google speech endpoint.
- Не открывает `/google-asr` и не запускает browser `SpeechRecognition` worker.
- Требует `asr.google_legacy_http.enabled = true`.
- `endpoint_host` остаётся advanced override; regular Google Cloud Speech API key path не требуется.
- Этот режим неофициальный, неподдерживаемый, отправляет аудио в Google и может перестать работать без предупреждения.
...
- remote worker sync также предотвращает уход в experimental legacy HTTP provider path.
