...
- явные миграции конфигурации (config migrations) и экспорт JSON Schema;
- модульный фронтенд панели управления (dashboard) на ES modules без сборки (без build step);
- маршрутизация перевода теперь управляется через стабильные слоты `translation_1 .. translation_5` (по слотам, а не только по коду языка);
- редактор настроек провайдера умеет «следовать» за выбранным слотом перевода и предупреждает, если у включённых линий не хватает обязательных параметров (ключей/endpoint/model);
- существенная стабилизация жизненного цикла Browser Speech и путей событий runtime/WebSocket;
- best-effort client-event logging без backend `500` из-за live event log;
- desktop launcher/bootstrap logs и runtime models теперь консистентно живут под `user-data/`, а legacy root `logs/` автоматически мигрирует туда;
...
5. В стартовом окне (launcher) выберите профиль запуска:
   - `Quick Start (Browser Speech)` (название в UI оставлено на английском)
   - `NVIDIA GPU (CUDA)`
   - `CPU-only`
   - `Remote Controller`
   - `Remote Worker`
