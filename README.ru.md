...
VoiceSub **`0.5.1`** (текущая линия) — Windows desktop-приложение для стримеров, которым нужны субтитры в реальном времени с опциональным переводом. Объединяет browser speech recognition, стилизацию, маршрутизацию и вывод в OBS в одном local-first процессе. По умолчанию bind `127.0.0.1:8765` — без cloud backend и аккаунтов.

Преемник SST Desktop `0.4.4`; baseline первого релиза VoiceSub — **`0.5.0`**. Стек: **Rust + Tauri**, **Svelte dashboard**, **vanilla OBS overlay**.
...
- **TTS-модуль** — native/Sonic dual-sink; озвучка субтитров + Twitch chat TTS (до **5 IRC-каналов** на одно OAuth, фильтры применяются на лету)
- Экспорт diagnostics ZIP (redacted config + логи)
- Локали UI: **en, ru, ja, ko, zh**
- Компактный макет под второй монитор / узкое окно

**Не в core 0.5.x:** локальный Parakeet ASR, experimental browser routes (архив в `legacy/`).
...
1. Установите **VoiceSub** из `VoiceSub_0.5.1_x64-setup.exe` (или последней сборки в папке релиза; разработчикам: `build-release-msi.bat` → `build-release.ps1`).
...
Активная разработка: Parakeet как опциональный **sidecar-модуль** после 0.5.x. Текущая patch-линия **`0.5.1`** — см. [CHANGELOG](./docs/CHANGELOG.md). Roadmap: `docs/plans/voicesub_roadmap.ru.md`.
...
Версия: `voicesub-types::PROJECT_VERSION` = **`0.5.1`**.
