...
- `v4`: добавление `asr.google_legacy_http`;
- `v5`: keyless legacy HTTP contract + `pair_id_prefix`.
...
### 10. Experimental Google Legacy HTTP Speech provider

В релиз входит и новый backend ASR provider:

- `google_legacy_http_experimental`

Он:

- работает внутри backend audio pipeline;
- не открывает `/google-asr`;
- не заменяет `browser_google`;
- не удаляет локальный Parakeet path;
...
UI/diagnostics path для него тоже включён в `0.3.0`.
...
- `google_legacy_http_experimental` остаётся unofficial provider path;
