# SST Desktop 0.4.2 — Технический документ

Актуально для линии кода, где `backend/versioning.py` содержит `PROJECT_VERSION = "0.4.2"`.
...
- встроенные пресеты: `clean_default`, `streamer_bold`, `dual_tone`, `compact_overlay`, `soft_shadow`, `meeting_soft`, `vlog_pastel`; тематические — `anime_stream` (Mochiy Pop One + Underdog для кириллицы), `retro_terminal` (VT323 + PT Mono), `fallout_pipboy` (Share Tech Mono + Ubuntu Mono), `comic_burst` (Bangers + Comic Relief), `cyberpunk_neon` (Orbitron + Exo 2), `noir_typewriter` (Special Elite + Cutive Mono), `dark_cinema` (Playfair Display), `accessibility_high_contrast` (Montserrat WCAG AAA);
- браузер выбирает шрифт из цепочки `font-family` по-глифно, поэтому тематические Latin-only фонты остаются основными, а Cyrillic-совместимые counterparts ниже по цепочке покрывают русский/украинский/болгарский текст без потери визуальной идентичности;
...
- `backend/versioning.py::PROJECT_VERSION = "0.4.2"` — single source of truth.
...
Текущий результат для `0.4.2+` (полное dev-дерево с `desktop/`):

- **462** collected tests (`python -m unittest discover -s tests -p "test_*.py"`);
- **461** OK при исключении pre-existing import error в `tests/test_browser_asr_observability.py` (`from tests.test_translation_dispatcher` — loader failure, не связан с desktop/runtime refactor);
...
- packaged release folders (после publish): `dist/desktop-releases/v0.4.2/` (`01-bootstrap-onefile/`, `01-bootstrap-web-only-onefile/`, `02-managed-app-onefolder/`, `03-installers-both/`)
...
## 21. Продуктовые инварианты, сохранённые в 0.4.2
...
- **translation lifecycle** (§12.3): completed translation block не снимается первым partial новой фразы; subtitle renderer fast path **сохраняет** эту инвариантность — `lifecycle_state` дотягивается от backend до `SubtitleStyleRenderer.render`, и переход transient→completed для source-блока не пересоздаёт completed translation в DOM;
...
**0.4.2 engineering hardening (без смены публичных API-контрактов):**
...
- subtitle renderer: incremental effects + shape-signature fast path в `frontend/js/subtitle-style.js` — эффект применяется только к свежим фрагментам, fast path переиспользует wrapper/stage/row/surface DOM между partial-кадрами, slow path сохраняет completed source surface при добавлении строки перевода;
- subtitle styles: 10 тематических пресетов в `backend/core/subtitle_style.py` с кириллически-безопасным fallback-цепочкой; 28 popular Google Fonts в `fonts/`;
...
- opt-in deep diagnostics: `backend/core/diagnostic_flags.py` + `SST_DEEP_DIAGNOSTICS` / `SST_TRACE_*` — JSONL-traces и `runtime_lifecycle.*` события по умолчанию выключены, по умолчанию logs верности 0.4.1 (см. `docs/ETALON_RUNTIME_VERIFICATION.md`).
