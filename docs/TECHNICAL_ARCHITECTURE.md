...
- [21.1 Линия 0.4.4](#211-линия-044)
...
- `desktop/launcher.py` — тонкий facade (re-export); **PyInstaller entrypoint** — обязателен `if __name__ == "__main__": main()` (иначе `.sst-runtime.exe` импортирует модуль и сразу выходит с code 0); `DesktopLauncher` / `main()` — в `desktop/launcher_bootstrap.py` (bootstrap, launch options, config, `run`/`shutdown`); mixins: `desktop/launcher_window.py` (`LauncherWindowMixin` — resize, `load_url`, splash DOM), `desktop/launcher_backend.py` (`LauncherBackendMixin` — subprocess/in-process backend, metrics monitor), `desktop/browser_worker_launcher.py` (`BrowserWorkerLauncherMixin`); константы/HTTP-хелперы и splash i18n (`_SPLASH_I18N`, `_splash_profile_hint`, UTF-8 literals) — `desktop/launcher_context.py`; pywebview API — `desktop/launcher_api.py`. Unit-тесты, патчащие символы внутри `DesktopLauncher`, должны бить в `desktop.launcher_bootstrap` / `desktop.launcher_backend`, а не только в re-export `desktop.launcher`.
...
- `desktop/runtime_bootstrap.py` — managed runtime + auto-detect install profile (CPU/NVIDIA); `_ensure_pip_available()` делегирует в `backend/bootstrap_pip_pins.ensure_pip_bootstrap()` (reuse pip ≥24.0 без PyPI; иначе `ensurepip`, затем pin `pip==24.3.1` — **не** `pip install --upgrade pip` на latest).
...
- `start.bat` — default local startup; шаг `[4/7]` вызывает `python -m backend.bootstrap_pip_pins --ensure-pip` (та же pip-политика, что desktop bootstrap).
...
Лаунчер/bootstrap: `test_launcher.py`, `test_launcher_module_layout.py`, `test_bootstrap_*.py`, `test_desktop_profile_lock.py`, `test_desktop_launcher_*.py`, `test_run_bind_policy.py`, `test_bootstrap_pip_pins.py` — исходники в `tests/`, импортируют модули из tracked `desktop/`.
...
- vendored `antlr4-python3-runtime==4.9.3` wheel (`vendor/python-wheels/`) + `backend/bootstrap_pip_pins.py` — antlr4 pin перед NeMo; с 0.4.4 также `ensure_pip_bootstrap()` / `--ensure-pip` (§21.1);
...
**Backlog (публичный CHANGELOG):** SSRF guard для OpenAI helper routes (`outbound_url_policy.py`); desktop slice в `store.js` + `patchDesktopContext()`; overlay WS stale/reconnect через `ws-stale-guard-logic.js`; split `desktop/launcher.py` на модули (`launcher_bootstrap`, `launcher_window`, `launcher_backend`, `launcher_context`, `launcher_api`, `browser_worker_launcher`); dashboard bootstrap-errors banner; `escapeHtml` в compact nav; тесты bind/profile (`resolve_bind_host`, `ProfileManager.resolve`); **UI i18n** — локали `ja`/`ko`/`zh`, bundle+dynamic архитектура, синхронная смена языка (§16.8); **ASR Advanced** — field help `?`, двухколоночная сетка, «рекомендуемое» (§16.6.1); **idle dashboard preview** — placeholder до Start после Save (§16.7.6).

**Bootstrap/runtime maintenance (только TECH, не product changelog):**

- **PyInstaller entrypoint:** `desktop/launcher.py` обязан содержать `if __name__ == "__main__": main()` — иначе frozen `.sst-runtime.exe` импортирует facade и выходит с code 0 без splash (`test_launcher_module_layout.py`).
- **Splash i18n:** строки `_SPLASH_I18N["ru"]` и `_splash_profile_hint()` живут в `desktop/launcher_context.py` как UTF-8 literals; при split нельзя переносить через lossy encoding.
- **Pip bootstrap policy:** `backend/bootstrap_pip_pins.py` — `BOOTSTRAP_PIP_MIN_VERSION=(24,0)`, `BOOTSTRAP_PIP_INSTALL_VERSION="24.3.1"`, `ensure_pip_bootstrap()` + CLI `--ensure-pip`. Reuse существующего pip без PyPI; иначе `ensurepip`, затем pin. Используется из `desktop/runtime_bootstrap.py` и `start.bat` `[4/7]`. **Не** `pip install --upgrade pip` на latest (избегает минутных PyPI timeout при уже установленном pip).

Инварианты §21 (0.4.3) и §12.3 / §16.7 **сохранены**.
