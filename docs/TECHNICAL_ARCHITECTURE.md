# SST Desktop 0.3.2 — Технический документ

Актуально для линии кода, где `backend/versioning.py` содержит `PROJECT_VERSION = "0.3.2"`.
...
│   └── assets/
├── tests/                        # unittest suite (298+ тестов; см. §20)
...
Детально — §11. Транскрипты поступают как `TranscriptEvent` с уже заполненным текстом; дальнейший путь через `TranscriptController` совпадает с локальным ASR (включая §7.4).
...
## 21. Продуктовые инварианты, сохранённые в 0.3.2
