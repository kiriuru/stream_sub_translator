...
│   ├── bootstrap_launcher_web_only.py  # Only Web.exe entry (--web-speech-only)
│   ├── runtime_bootstrap.py
│   ├── bootstrap_payload.py
│   ├── build_bootstrap_payload.py
│   └── assets/
├── tests/                        # unittest suite (306+ тестов; см. §20)
├── docs/                         # CHANGELOG, TECHNICAL_ARCHITECTURE, release notes
├── start.bat, start-remote-*.bat
├── build-desktop.bat
├── build-bootstrap-launcher.bat, build-bootstrap-launcher-web-only.bat
...
- **Старт dashboard (`0.4.0`):** `main.js` монтирует панели сразу; `DesktopBridge.getContext()` и `actions.loadInitialData()` выполняются в фоне (не блокируют shell на `pywebviewready`).
...
Текущий результат для `0.4.0`:

- **336** tests
- `OK`
- включая `tests/test_desktop_profile_lock.py`, `tests/test_browser_asr_observability.py`

Расширенная команда верификации для `0.4.0`:
...
- лаунчере и bootstrap (`test_launcher.py`, `test_bootstrap_launcher.py`, `test_bootstrap_payload.py`, `test_runtime_bootstrap.py`, `test_desktop_profile_lock.py`);
