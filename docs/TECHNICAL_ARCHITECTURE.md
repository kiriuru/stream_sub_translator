...
- [5. Backend layout (детально)](#5-backend-layout-детально)
- [6. RuntimeOrchestrator: тонкий фасад и lifecycle](#6-runtimeorchestrator-тонкий-фасад-и-lifecycle)
...
- [10. Логи, диагностика, экспорт](#10-логи-диагностика-экспорт)
- [11. Browser Speech: классический и experimental пути](#11-browser-speech-классический-и-experimental-пути)
- [12. Перевод: lifecycle и инварианты](#12-перевод-lifecycle-и-инварианты)
- [13. Стили субтитров (backend config + presets)](#13-стили-субтитров-backend-config--presets)
- [14. Desktop runtime и release](#14-desktop-runtime-и-release)
- [15. Хранилище и пути](#15-хранилище-и-пути)
- [16. Frontend (плоско, без build-step)](#16-frontend-плоско-без-build-step)
- [16.8 UI localization (i18n)](#168-ui-localization-i18n)
- [17. Локальный ASR: Official EU Parakeet Low Latency (NeMo)](#17-локальный-asr-official-eu-parakeet-low-latency-nemo)
- [18. Remote mode (frozen surface)](#18-remote-mode-frozen-surface)
- [19. Версионирование и проверка обновлений](#19-версионирование-и-проверка-обновлений)
- [20. Тестирование](#20-тестирование)
- [21. Продуктовые инварианты, сохранённые в 0.4.3](#21-продуктовые-инварианты-сохранённые-в-043)
...
| Endpoint | Назначение |
| --- | --- |
| `POST /api/runtime/start` | Старт рантайма (опционально с in-memory config snapshot) |
| `POST /api/runtime/stop` | Остановка рантайма |
| `GET /api/runtime/status` | Текущий runtime status и diagnostics snapshot |
| `POST /api/settings/save` | Нормализация + атомарное сохранение `config.json` |
...
| Channel | Назначение |
| --- | --- |
| `/ws/events` | Dashboard + overlay события (runtime/subtitle payload) |
| `/ws/asr_worker` | Browser Speech worker transport |
| `/ws/remote/*` | Remote controller/worker signaling и ingest |
...
| `backend/core/runtime_orchestrator.py` | Тонкий фасад рантайма |
| `backend/core/subtitle_lifecycle_core.py` | FSM/TTL/relevance для subtitle lifecycle |
| `backend/core/translation_dispatcher.py` | Slot-aware очередь перевода + stale drop |
| `frontend/js/subtitle-style.js` | Общий renderer для dashboard preview и overlay |
...
Ключевые инварианты hot path:

- `SegmentQueue` в streaming path передаёт дельты PCM (`audio_is_delta`), а не полный буфер сегмента.
- `SubtitleLifecycleCore` не снимает completed translation на первом partial новой фразы.
- `OverlayBroadcaster` держит cooldown для `completed_only`, чтобы не засыпать overlay одинаковыми кадрами.
...
| `profile_manager.py` | Профили (`user-data/profiles/`), нормализация payload, default profile; `_profile_path()` — `Path.resolve()` + `is_relative_to(profiles_root)` против path traversal (`..`, `/`, `\` в имени). |
| `bind_policy.py` | `resolve_bind_host()` — explicit `--host` > `0.0.0.0` при `--allow-lan` > default `127.0.0.1`; используется из `backend/run.py`. |
| `outbound_url_policy.py` | SSRF-политика для OpenAI helper routes (§8); translation provider outbound URLs не затронуты. |
...
- встроенный JSON (`backend/data/source_text_builtin_pairs.json`) покрывает **английский, русский, японский, корейский и китайский** (латиница + CJK); UI-чекбокс «встроенный список» отражает все пять языков;
...
- `desktop/launcher.py` — тонкий facade (re-export); **PyInstaller entrypoint** — обязателен `if __name__ == "__main__": main()` (иначе `.sst-runtime.exe` импортирует модуль и сразу выходит с code 0 без splash; регрессия `test_launcher_module_layout.py`); `DesktopLauncher` / `main()` — в `desktop/launcher_bootstrap.py` (bootstrap, launch options, config, `run`/`shutdown`); mixins: `desktop/launcher_window.py` (`LauncherWindowMixin` — resize, `load_url`, splash DOM), `desktop/launcher_backend.py` (`LauncherBackendMixin` — subprocess/in-process backend, metrics monitor), `desktop/browser_worker_launcher.py` (`BrowserWorkerLauncherMixin`); константы/HTTP-хелперы и splash i18n (`_SPLASH_I18N`, `_splash_profile_hint`) — `desktop/launcher_context.py` (**UTF-8 literals**; при split нельзя переносить через lossy encoding); pywebview API — `desktop/launcher_api.py`. Unit-тесты, патчащие символы внутри `DesktopLauncher`, должны бить в `desktop.launcher_bootstrap` / `desktop.launcher_backend`, а не только в re-export `desktop.launcher`.
...
- `desktop/runtime_bootstrap.py` — managed runtime + auto-detect install profile (CPU/NVIDIA); `_ensure_pip_available()` делегирует в `backend/bootstrap_pip_pins.ensure_pip_bootstrap()`.
- `backend/bootstrap_pip_pins.py` — pip bootstrap policy: `BOOTSTRAP_PIP_MIN_VERSION=(24,0)`, `BOOTSTRAP_PIP_INSTALL_VERSION="24.3.1"`; reuse существующего pip ≥24.0 **без PyPI**, иначе `ensurepip`, затем pin; CLI `python -m backend.bootstrap_pip_pins --ensure-pip`. **Не** `pip install --upgrade pip` на latest (избегает минутных PyPI timeout). Также vendored `antlr4-python3-runtime==4.9.3` pin перед NeMo (`vendor/python-wheels/`).
...
- `start.bat` — default local startup; шаг `[4/7]` вызывает `python -m backend.bootstrap_pip_pins --ensure-pip` (§14.1).
...
- `backend/run.py` — общий runtime launcher с `--remote-role`, `--allow-lan` и `resolve_bind_host()` (`backend/core/bind_policy.py`);
...
- **Компактный layout (`0.4.0`):** `ui.layout` ∈ `{standard, compact}`; `frontend/css/compact-layout.css`, `frontend/js/layout/layout-controller.js` (icon rail, sticky chrome; labels nav-кнопок через `escapeHtml()` для локализованных/CJK подписей); desktop shell ресайзит окно при смене layout (standard ~1440×940, compact ~400×844).
- **Bootstrap load errors:** `frontend/js/dashboard/actions/data-actions.js` → `loadInitialData()` параллельно тянет version/health/obs/settings/audio; частичные сбои агрегируются в `store.ui.saveStatus` (`bootstrap.incomplete`, per-endpoint `bootstrap.error.*`); shell не падает если settings загрузились (warn tone), fatal если settings недоступны.
- `frontend/js/i18n.js` — runtime-каталоги UI-строк (`en`, `ru`, `ja`, `ko`, `zh`); см. **§16.8**;
...
- `frontend/js/core/`:
  - `store.js` — центральный snapshot state + `subscribe` / `subscribeSelector`:
    - slice **`desktop`** + **`patchDesktopContext()`** — единый источник desktop-контекста (mode, paths, launch metadata); `main.js` слушает `sst:desktop-context` один раз;
  ...
  - `api-client.js`, `ws-client.js` (stale-filter через `ws-stale-guard-logic.js`, reconnect exponential backoff 1–10 s — тот же алгоритм, что overlay §16.4), `events.js`, `redaction.js`;
...
- подключается к `/ws/events`; stale-фильтр через `frontend/js/core/ws-stale-guard-logic.js` (тот же алгоритм, что `WsClient`: timestamp-first при reset sequence после stop/start); reconnect с exponential backoff 1–10s; при disconnect последний кадр сохраняется до reconnect;
...
- в тюнинге — «ощущающие» ползунки; точные тайминги/гейты ASR — вкладка **ASR Advanced** (`data-tab-panel="asr_advanced"`);
- статус `experimental` для экспериментальных провайдеров перевода остаётся `experimental` (не `degraded`);
- смена языка UI — синхронно, без lazy-load; при выборе в header/settings вызывается `saveCurrentConfig()` → `ui.language` в `user-data/config.json` + `localStorage` (`sst.ui.language`); см. **§16.8**.
...
| `overlay-panel` preview | `SubtitleStyleRenderer.render` на `#subtitle-output-preview`; **caller** очищает `.subtitle-stage-note` и вызывает `disposeRenderContainer` при unmount/пустом preview (§16.7). Idle placeholder (`buildPreviewPayload` в `action-helpers.js`) сохраняется до **Start**, даже если WS replay прислал пустой `overlay_update` после Save. |
...
### 16.6.1 ASR advanced — контекстная справка (dashboard)

Вкладка **`asr_advanced`** (`frontend/index.html`, mount через `frontend/js/panels/asr-panel.js`) содержит точный Parakeet realtime-тюнинг (VAD, partial emit, segmentation, energy gate, Parakeet-only extras: latency preset, streaming decode, partial emit mode).

| Элемент | Файл / ключи | Поведение |
| --- | --- | --- |
| Кнопка `?` | `.field-help-btn[data-field-help-key]` | В конце названия каждого поля (в т.ч. checkbox-row). `aria-expanded`, `data-i18n-aria-label="tools.advanced.field_help.aria"`. |
| Popover | `frontend/js/ui/field-help-popover.js` | `position: fixed`; **нижняя граница** popover совпадает с нижней границей кнопки (рост вверх). Текст из `I18n.t(helpKey)`. Стили: `--bg-panel-elevated`, `--text-primary`, `--line-subtle`, `--shadow-panel` (поддержка light/dark через `data-ui-theme` и `ui-theme.js`). Toggle по клику на ту же кнопку; закрытие — outside click, `Escape`, `resize`, unmount panel. |
| Рекомендуемое значение | `tools.advanced.<field>.note` | Одна строка справа от названия (`.inline-field-note`): «Рекомендуемое: …» / локализованный префикс + значение. Заменяет старый формат `def:… safer:…`. |
| Справка по полю | `tools.advanced.<field>.help` | Короткое объяснение назначения и эффекта настройки на ASR (plain language). |
| Пресет задержки | `tools.advanced.latency_preset.help/note` | В тексте и note используются **локализованные** labels из `tuning.preset.ultra_low_latency` / `balanced` / `quality`, не английские slug. |

| Layout | `.asr-advanced-fields-grid` | Двухколоночная CSS grid в standard; в **compact** — одна колонка (`compact-layout.css`). Parakeet-only блок `#rt-tools-local-parakeet-extras` с `display: contents`, скрытие через `.is-hidden`. |

**Mount:** `mountFieldHelpButtons(document.querySelector('[data-tab-panel="asr_advanced"]'), t)` в `bindAsrEvents`; cleanup при unmount панели.

**i18n:** все `tools.advanced.*.help` и `.note` присутствуют в `frontend/js/i18n/locales/{en,ru,ja,ko,zh}.js`; bundle — `locales-bundle.js`. Пакетное обновление CJK note/help: `tools/patch_asr_advanced_i18n_cjk.py`. Ключи `tools.notes.*` удалены (дублировали справку полей).

**Регрессии:** `tests/test_field_help_popover.py` (наличие кнопок, ключи en/ru, локализация ja/ko/zh, локализованные имена пресетов, формат note, двухколоночная разметка без side notes).

**Compact layout:** `.inline-field-note` **не** скрывается в compact (см. `tests/test_frontend_architecture.py`).
...
- payload из `buildPreviewPayload()` — live `overlay.payload` только когда runtime **running** или в payload есть renderable текст; иначе style placeholder («Предпросмотр исходной строки» / labels переводов) для настройки до Start и после Save без запущенного ASR;
...
Плоский vanilla-i18n без Node/Webpack: все строки dashboard, Browser Speech worker, overlay и большинство подсказок идут через `window.I18n.t(key)` или атрибуты `data-i18n*`. **Нет** отдельного HTTP-загрузчика локалей — каталоги предзагружаются синхронными `<script>` до `i18n.js`, чтобы смена языка была мгновенной.

#### 16.8.1 Эволюция архитектуры (0.4.4)

| Этап | Состояние |
| --- | --- |
| До 0.4.4 | Два встроенных словаря `ru`/`en` внутри `frontend/js/i18n.js`; ветвления `=== "ru"` в панелях. |
| 0.4.4 | **Раздельные locale-файлы** + **динамический слой** для поздних ключей + **единый bundle** для надёжной загрузки во встроенном WebView2. |
| Инвариант | Один ключ → одна строка в каталоге; панели не хранят параллельные копии переводов. |

Мотивация смены: (1) добавить **ja / ko / zh** без раздувания `i18n.js`; (2) убрать гонки 404 при отдельных `fetch`/`en.js`+`ru.js` в desktop shell; (3) покрыть ~115 «поздних» ключей (overlay hints, ASR transcript empty states, `tools.advanced.*.note`, format-строки с `{placeholders}`) без дублирования в каждом locale-файле вручную для en/ru.

#### 16.8.2 Файлы и слои данных

```
frontend/js/i18n/
├── locales/
│   ├── en.js, ru.js, ja.js, ko.js, zh.js   # window.__SST_I18N_LOCALES.<code> (JSON object)
├── locales-bundle.js                        # сборка всех locales (генерируется)
├── dynamic-locales.js                       # window.__SST_I18N_DYNAMIC { en, ru }
└── (runtime) ../i18n.js                     # merge + I18n API

desktop/ui_locale.py                         # normalize_ui_language() для splash/config (Python)
tools/build_i18n_locale_bundle.py            # locales/*.js → locales-bundle.js
tools/build_dynamic_i18n.py                  # обновление en/ru в dynamic-locales.js
tools/generate_i18n_locales.py               # ja/ko/zh из полного en-каталога (перевод)
tools/fix_untranslated_cjk.py                # дозаполнение ключей, где locale == en
```

| Слой | Содержимое | Локали |
| --- | --- | --- |
| `locales/<code>.js` | Основной словарь (~697 ключей на 0.4.4) | en, ru, ja, ko, zh |
| `dynamic-locales.js` | Ключи, добавленные после стабилизации статического `en.js` (overlay preset hints, списки, лог-сообщения UI) | **только** `en` и `ru` в файле; ja/ko/zh получают те же ключи через `generate_i18n_locales.py` в свои `.js` |
| `i18n.js` | Merge + `t()` / `apply()` / `setLocale()` | все пять |

**Порядок слияния каталога** (`getCatalog(locale)` в `i18n.js`):
...
Следствие: если ключ есть только в `dynamic.en` и **отсутствует** в `ja.js`, UI на японском покажет **английский fallback** из базового english-каталога. После генерации CJK-файлов обязателен прогон `fix_untranslated_cjk.py` для ключей, где машинный перевод вернул исходный английский текст.

#### 16.8.3 Загрузка на страницах (порядок скриптов)

Обязательный порядок (dashboard `frontend/index.html`, `overlay/overlay.html`, `frontend/google_asr*.html`):

1. `locales-bundle.js` — заполняет `window.__SST_I18N_LOCALES`
2. `dynamic-locales.js` — заполняет `window.__SST_I18N_DYNAMIC`
3. `i18n.js` — строит каталоги, первичный `apply(document)`
4. ES-модули (`main.js` и т.д.) — импортируют `t` / `getCurrentLocale` из `dashboard/helpers.js`

Query-параметр `?v=…` на i18n-скриптах (**cache bust**) обязателен при релизе: встроенный WebView2 агрессивно кеширует `/static/js/…`. Без бампа пользователь видит старый bundle (типичный симптом — английские overlay hints при выбранной ja).

`main.js` подключается отдельным модулем и **не** блокирует parse i18n; панели монтируются после `I18n` уже инициализирован.

#### 16.8.4 Публичный API и персистентность

`window.I18n`:

| Метод / поле | Назначение |
| --- | --- |
| `t(key, variables?)` | Перевод; `{name}` подстановки через regex; отсутствующий ключ → humanize последнего сегмента ключа |
| `setLocale(code)` | Синхронная смена + `apply(document)` + `CustomEvent("sst:locale-changed")` |
| `getLocale()` | Текущий код |
| `apply(root?)` | Обход `data-i18n`, `data-i18n-placeholder`, `data-i18n-title`, `data-i18n-aria-label` |
| `supported` | `["en","ru","ja","ko","zh"]` |

**Два источника выбора языка при старте:**

1. `localStorage["sst.ui.language"]` — если значение ∈ `supported`;
2. иначе `navigator.language` → `resolveBrowserLocale()` (`ru*`, `ja*`, `ko*`, `zh*` → соответствующий код).

**Конфиг:** `ui.language` в `ConfigSchema` / `backend/data/config.schema.json` — enum `"" | en | ru | ja | ko | zh`. Нормализация: `frontend/js/normalizers/config-normalizer.js` (`normalizeUiLanguage`) и `desktop/ui_locale.py` (`normalize_ui_language`) — одинаковый набор кодов.

**Смена из UI:** `frontend/js/shell/locale-switcher.js` → `actions.setUiLanguage()` (`config-actions.js`):

- `I18n.setLocale(nextLocale)`;
- `mutateConfig` → `draft.ui.language = nextLocale`;
- `saveCurrentConfig()` — запись на диск без отдельной кнопки Save для языка;
- `events.emit("locale:changed")` (внутренний bus dashboard).

При `loadInitialData` / `setConfig` язык из сохранённого конфига применяется через `I18n.setLocale`, если отличается от текущего.

#### 16.8.5 Событие `sst:locale-changed` и панели

`I18n.setLocale` диспатчит **нативное** `window` событие `sst:locale-changed` (не путать с `locale:changed` на dashboard `events`).

Подписчики обязаны **перерисовать** динамический DOM, не покрытый `data-i18n`:

| Модуль | Поведение |
| --- | --- |
| `translation-panel.js` | Сброс `lastResultsRenderKey`, повторный `renderTranslationResults` (пустое состояние «нет переводов» и заголовки) |
| `translation-panel.js` | `resultsKey` включает `getCurrentLocale()` |
| `asr-panel.js`, `style-editor-panel.js`, `layout-controller.js` | Перерисовка подписей / compact nav |
| `field-help-popover.js` (ASR advanced) | Обновление текста открытого popover на `sst:locale-changed` |
| `main.js` | Синхронизация `#ui-language-select` и `store.ui.uiLanguage` |
| `shell/help-topics.js` | Перезагрузка help-топиков |
| `overlay/overlay.js`, `google_asr*.html` | Повторный `I18n.apply` |

Импорт строк в ES-модулях: `import { t, getCurrentLocale } from "../dashboard/helpers.js"` (обёртка над `window.I18n`). **Регрессия 0.4.4:** `style-editor-panel-render.js` без импорта `t` давала `ReferenceError` при mount.

#### 16.8.6 Что остаётся на английском

| Область | Политика |
| --- | --- |
| Runtime/deep diagnostics в UI | `helpers.js` — English-only токены для deep diagnostic labels (все UI-локали) |
| Технические идентификаторы в подсказках | Допустимы латиница для enum/slug в `<select>` (`word_growth`, `char_delta`); user-facing help/note для пресетов Parakeet используют `tuning.preset.*` labels |
| Desktop splash до dashboard | `desktop/launcher_context.py` — `_SPLASH_I18N` пока **en/ru** literals на pywebview splash; CJK splash — не в scope 0.4.4 |
| Backend logs / JSONL traces | Без i18n |

#### 16.8.7 Сопровождение переводов (разработчик)

1. Новая строка UI → ключ `domain.section.label` в `frontend/js/i18n/locales/en.js` (и `ru.js` для ru).
2. Если ключ относится к overlay/ASR/tools notes — также в `tools/build_dynamic_i18n.py` / `dynamic-locales.js` для en+ru.
3. `python tools/generate_i18n_locales.py` — обновить `ja.js`, `ko.js`, `zh.js` от полного en-каталога (en.js ∪ dynamic.en).
4. `python tools/fix_untranslated_cjk.py` — повтор для ключей, где перевод совпал с английским (сетевой вызов Google translate, retry на SSL).
5. `python tools/build_i18n_locale_bundle.py` — пересобрать `locales-bundle.js`.
6. Бамп `?v=` на `locales-bundle.js`, `dynamic-locales.js`, `i18n.js` во всех HTML entrypoints.
7. Тесты: `tests/test_i18n_locales.py`, `tests/test_i18n_dynamic_locales.py`, `tests/test_ui_locale.py`, `tests/test_field_help_popover.py` (ASR advanced note/help).

Проверка покрытия: `python tools/_list_cjk_same_as_en.py` — список user-facing ключей, где ja/ko/zh всё ещё идентичны en.

#### 16.8.8 Регрессии (0.4.4)

- `test_index_eager_loads_locale_bundle_before_i18n` — порядок скриптов в `index.html`;
- `test_cjk_locale_files_cover_full_english_catalog` — ja/ko/zh содержат все ключи merged en;
- `test_cjk_user_facing_strings_use_target_script` — выборочные строки содержат японскую/корейскую/китайскую письменность;
- `test_dynamic_keys_merged_at_build_time` — контракт merge;
- `test_ui_locale.py` — Python-нормализация `ja-JP` → `ja`, и т.д.
...
- frontend contracts: `test_frontend_modular_vanilla.py` (store emit isolation, dom helpers, panel bindings, config normalizer `continuous_results`, translation panel imports), `test_ws_stale_guard.py`, `test_frontend_architecture.py`
...
- packaged release folders (после publish): `dist/desktop-releases/v0.4.4/` (`01-bootstrap-onefile/`, `01-bootstrap-web-only-onefile/`, `02-managed-app-onefolder/`, `03-installers-both/`)
...
Лаунчер/bootstrap: `test_launcher.py`, `test_launcher_module_layout.py`, `test_bootstrap_*.py`, `test_desktop_profile_lock.py`, `test_desktop_launcher_*.py`, `test_bind_policy.py`, `test_profile_manager_paths.py`, `test_bootstrap_pip_pins.py` — исходники в `tests/`, импортируют модули из tracked `desktop/`.
...
- vendored `antlr4-python3-runtime==4.9.3` wheel + `backend/bootstrap_pip_pins.py` — antlr4 pin перед NeMo; `ensure_pip_bootstrap()` / `--ensure-pip` (§14.1);
...
### 22.1 Текущие ограничения

- Remote mode остаётся frozen surface (§18): поддержка сохранена, но не является активным направлением развития.
- Browser Speech path зависит от политики браузера/ОС энергосбережения; hardening снижает риск деградации, но не даёт абсолютной гарантии на всех OEM-конфигурациях Windows.
- Local Parakeet требует заметных ресурсов (GPU-first); на слабом железе ожидается degraded поведение и рост latency.
- Overlay subtitle trace (`?debug-subtitles=1` / `localStorage.sst_debug_subtitles=1`) остаётся developer-facing инструментом без отдельного UI-toggle в панели.

### 22.2 Технический долг (не блокирующий)

- `backend/runtime_paths.py` остаётся shim над `backend/core/paths.py` для обратной совместимости импортов.
- `frontend/js/state.js` и `frontend/js/app.js` поддерживаются как legacy compatibility слой параллельно `frontend/js/core/*`.
- `translation.provider_limits` присутствует в схеме как расширяемая структура; текущий runtime использует её ограниченно и не как полноценную policy-DSL.

## 23. Security & Privacy Model

### 23.1 Local-first модель

- По умолчанию сервис привязывается к `127.0.0.1`; LAN bind включается только для явного remote worker сценария.
- Продукт не требует cloud backend, аккаунтов или обязательной внешней телеметрии.
- Пользовательские данные, логи, кеш и профили живут локально (`user-data/`, `logs/`).

### 23.2 Секреты и диагностика

- Конфиг и provider settings хранятся локально; экспорт diagnostics проходит через редактирование чувствительных полей (`backend/core/redaction.py`).
- `GET /api/exports/diagnostics` формирует redacted архив (конфиг/логи без сырого секрета в стандартном потоке).
- При повреждении JSON-файлов используется quarantine-подход (`*.corrupt-<timestamp>.json`) вместо опасного silent truncate/delete.

### 23.3 Browser worker и desktop bootstrap

- Browser Speech worker запускается в отдельном окне с изолированным `user-data-dir`, отдельно от основного профиля пользователя.
- WebSocket transport hardening включает bounded очереди и сериализацию `send_json` на один сокет (`/ws/events`, `/ws/asr_worker`).
- Desktop bootstrap извлекает managed runtime локально рядом с лаунчером; операция требует write-доступа к install-directory.

### 23.4 Remote (LAN) security assumptions

- Remote surface рассчитан на доверенную LAN среду и явный opt-in.
- Не рассматривать remote сценарий как internet-facing deployment; контроль доступа и firewall остаются обязанностью локальной инфраструктуры/ОС.

## 24. Extension Points (How To Extend Safely)

### 24.1 Добавить новый translation provider

1. Реализовать provider в `backend/translation/providers/` через `BaseTranslationProvider`.
2. Зарегистрировать в `backend/translation/registry.py`.
3. Расширить `TranslationProviderSettings` в `backend/schemas/config_schema.py` и normalizers в `backend/config/normalizers/translation.py`.
4. Подключить UI-настройки в `frontend/js/panels/translation-panel.js`.
5. Добавить/обновить тесты: provider unit tests + `tests/test_translation_dispatcher.py` (контракты lifecycle/relevance).

### 24.2 Добавить новый subtitle effect

1. Добавить CSS effect в `frontend/css/subtitle-style.css`.
2. Разрешить effect в backend в `backend/core/subtitle_style.py`.
3. Синхронизировать validation/normalization на фронте в `frontend/js/normalizers/overlay-normalizer.js`.
4. Добавить option в style editor (`frontend/js/panels/style-editor-panel.js`).
5. Обязательно обновить `tests/test_subtitle_style_effects.py`.

### 24.3 Добавить новый ASR mode/source

1. Реализовать speech source в `backend/core/runtime/`.
2. Зарегистрировать mode в `speech_source_factory.py` и/или `asr_mode_controller.py`.
3. Обновить схему/нормализацию (`backend/schemas/asr_schema.py`, `backend/config/normalizers/asr.py`).
4. Обновить UI selector в `frontend/js/panels/asr-panel.js`.
5. Добавить тесты runtime ingress/status + убедиться, что translation/subtitle lifecycle инварианты не нарушены.

## 25. Performance Characteristics (Operational Expectations)

### 25.1 Что считать baseline

- Документ фиксирует архитектурные ожидания и относительные характеристики, а не аппаратно-независимые SLA.
- Фактические значения latency/CPU/GPU зависят от режима (`local`, `browser_google`, `browser_google_experimental`), модели, выбранных провайдеров перевода и железа.
- Для диагностики использовать `runtime_status`, `diagnostics_update`, `runtime-events.log`, `session-latest.jsonl`.

### 25.2 Практические ожидания по режимам

- `local` + Parakeet (GPU): минимальная end-to-end задержка и наилучший UX на поддерживаемой CUDA-конфигурации.
- `local` + CPU fallback: функционально поддерживается, но воспринимается как degraded mode по latency/throughput.
- Browser Speech modes: latency зависит от браузерного движка и сетевых условий; стабильность повышается за счёт supervisor FSM и restart policy.
- Translation добавляет асинхронную задержку поверх ASR; dispatcher снижает шум через stale drop и supersession checks.

### 25.3 Узкие места и наблюдаемость

- Hot path: audio capture -> VAD -> segment queue -> decode -> subtitle router -> overlay fanout.
- Пиковая нагрузка обычно связана с local ASR compute + параллельными сетевыми вызовами translation providers.
- При анализе просадок использовать связку: `event_sequence`, runtime diagnostics, browser worker generation/session и dispatcher observability.

## 26. Glossary

- **Partial**: промежуточный результат распознавания до финализации фразы.
- **Final**: финализированная фраза, которая уже может входить в translation path.
- **Slot**: идентификатор translation line (`translation_1..translation_5`), независимый от кода языка.
- **Generation**: monotonic идентификатор browser worker сессии для защиты от stale сообщений.
- **Coalescing**: подавление/объединение частых однотипных status update событий.
- **TTL**: время жизни completed блока в lifecycle.
- **FSM**: конечный автомат жизненного цикла субтитров.
- **Hot path**: критический путь данных, влияющий на end-to-end latency.
- **Idempotent DOM update**: обновление UI без лишнего сброса focus/caret и без лишней перерисовки.
- **Quarantine**: перенос повреждённого JSON-файла в `*.corrupt-<timestamp>.json` вместо безусловного удаления.
