...
from desktop.deps_install_trace import deps_trace
from desktop.subprocess_trace import subprocess_trace
...
def quiet_import_check_snippet(body: str) -> str:
    """Python -c body that exits 0/1 without printing tracebacks on failure."""
    indented = "\n".join(f"    {line}" for line in body.strip().splitlines())
    return f"import sys\ntry:\n{indented}\nexcept Exception:\n    sys.exit(1)\nsys.exit(0)"


_QUIET_VALIDATE_BASE = quiet_import_check_snippet(
    "import fastapi, uvicorn, pydantic, websockets, sounddevice, numpy, httpx, webrtcvad, webview\n"
    "assert hasattr(webview, 'create_window')"
)
_QUIET_VALIDATE_WEBVIEW = quiet_import_check_snippet(
    "import webview\n"
    "assert hasattr(webview, 'create_window')"
)
_QUIET_VALIDATE_NEMO = quiet_import_check_snippet(
    "import importlib\n"
...
def build_runtime_environment(paths: DesktopRuntimePaths, *, pythonpath_root: Path | None = None) -> dict[str, str]:
    ...
    elif (paths.bundle_root / "backend").is_dir():
        # Desktop managed installs keep Python sources under app-runtime/, not project_root/backend/.
        env["SST_PYTHONPATH_ROOT"] = str(paths.bundle_root)
    return env
...
class RuntimeBootstrapper:
    ...
    def ensure_backend_environment(self, *, install_profile_override: str | None = None) -> tuple[Path, str]:
        deps_trace("bootstrap", "ensure_backend_environment_begin", install_profile_override=install_profile_override)
        ...
        deps_trace(
            "bootstrap",
            "ensure_backend_environment_complete",
    ...
    def ensure_base_environment(self) -> Path:
        deps_trace(
            "bootstrap",
            "ensure_base_environment_begin",
            requirements_file=str(self._paths.runtime_base_requirements),
            installs="python,venv,pip,requirements.runtime.base.txt",
            skips="torch,nemo",
        )
        ...
        deps_trace("bootstrap", "ensure_base_environment_complete", venv_python=str(self._paths.venv_python))
        return self._paths.venv_python

    def ensure_local_asr_runtime(self, *, install_profile_override: str | None = None) -> str:
        deps_trace(
            "bootstrap",
            "ensure_local_asr_runtime_begin",
            install_profile_override=install_profile_override,
            installs="base+torch+requirements.runtime.ai.txt+nemo",
        )
        ensure_runtime_layout(self._paths)
        self._ensure_local_python()
        self._ensure_venv()
        ...
        self._verify_installed_local_asr_stack(install_profile)
        deps_trace(
            "bootstrap",
            "ensure_local_asr_runtime_complete",
            install_profile=install_profile,
            torch_requirements=str(
                self._paths.torch_cpu_requirements
                if install_profile == "cpu"
                else self._paths.torch_cuda_requirements
            ),
        )
    ...
    def _verify_installed_local_asr_stack(self, install_profile: str) -> None:
        snippet = quiet_import_check_snippet(
            "import fastapi, torch, torchaudio\n"
            "import importlib\n"
            "importlib.import_module('nemo.collections.asr.models')\n"
        ...
        completed = self._run_command(
            [str(self._paths.venv_python), "-c", snippet],
            description="verify installed local ASR stack",
            env=build_runtime_environment(self._paths),
            allow_failure=True,
        )
        if completed.returncode != 0:
            raise RuntimeError(
                f"Local ASR environment finished installing but failed verification for profile '{install_profile}'. "
                "Try Repair Runtime from the bootstrap splash or delete .venv and restart with CPU/GPU."
            )
        self._log(f"[deps] Verified local ASR stack for profile: {install_profile}")
    ...
    def _run_command(
        ...
        *,
        ...
        subprocess_trace(
            ...
            elapsed_ms=round((time.perf_counter() - command_started) * 1000.0, 2),
    ...
    def _torch_runtime_matches_profile(self, install_profile: str) -> bool:
        ...
        return self._run_quiet_python_check(
            description="validate existing torch profile",
            snippet=snippet,
            missing_message=(
                f"[deps] PyTorch runtime does not match the {install_profile} profile yet; installing..."
            ),
        )
    ...
    def _base_requirements_satisfied(self) -> bool:
        return self._run_quiet_python_check(
            ...
            snippet=_QUIET_VALIDATE_BASE,
            missing_message="[deps] Base desktop Python requirements are not installed yet; installing...",
        )
    ...
    def _desktop_shell_requirements_satisfied(self) -> bool:
        return self._run_quiet_python_check(
            description="validate desktop shell (pywebview) in local venv",
            snippet=_QUIET_VALIDATE_WEBVIEW,
            missing_message="[deps] Desktop shell (pywebview) is not available in the local venv yet; installing...",
        )

    def ensure_desktop_shell_requirements(self) -> None:
        if self._desktop_shell_requirements_satisfied():
            self._log("[deps] Desktop shell (pywebview) is already available in the local venv.")
            return
        self._status("Installing desktop shell dependencies...")
        self._run_command(
            [
            ...
            description="install desktop shell (pywebview) into local venv",
            env=build_runtime_environment(self._paths),
        )
        if not self._desktop_shell_requirements_satisfied():
            raise RuntimeError(
                "Desktop shell dependencies failed to install into the local venv. "
                "pywebview is required for the venv handoff path."
            )

    def _ai_requirements_satisfied(self) -> bool:
        return self._run_quiet_python_check(
            description="validate local AI recognition requirements",
            snippet=_QUIET_VALIDATE_NEMO,
            missing_message="[deps] Local AI recognition requirements are not installed yet; installing...",
        )

    def _ensure_antlr4_runtime(self) -> None:
        if antlr4_runtime_satisfied(self._paths.venv_python):
            self._log(f"[deps] antlr4-python3-runtime {ANTLR4_RUNTIME_VERSION} is already available.")
            return
        self._status("Installing bundled antlr4 runtime wheel...")
        install_bundled_antlr4_runtime(
    ...
    def _ensure_ai_requirements(self) -> None:
        ...
        self._run_command(
            [
                ...
                "--no-cache-dir",
                "-r",
                str(self._paths.runtime_ai_requirements),
            ],
    ...
    def _parakeet_model_file(self) -> Path:
        return (
    ...
    def _ensure_model_directory(self) -> None:
        ...
        model_file = self._parakeet_model_file()
        if model_file.exists():
            self._log(f"[model] Reusing local model file: {model_file}")
            return
        self._log(
            "[model] Official Parakeet model is not installed yet. "
            "The first Start in the dashboard will download it with progress shown on the Runtime panel."
        )
