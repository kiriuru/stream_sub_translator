...
from desktop.subprocess_trace import configure_subprocess_trace, logged_popen
...
class BootstrapLauncher:
    def __init__(self) -> None:
        ...
        self._rotate_log_file(self._paths.log_path)
        configure_subprocess_trace(self._paths.logs_dir, text_log=self._log)
        self._ui = BootstrapUi(APP_NAME)

    @staticmethod
    def _rotate_log_file(log_path: Path) -> None:
        """Preserve the previous run's bootstrap log to keep launch history for triage."""
    ...
    def _migrate_legacy_logs_dir(self) -> None:
        ...
        legacy_candidates = (
            self._paths.exe_dir / "logs",
            self._paths.exe_dir / "user-data" / "logs",
        )
        for legacy_logs_dir in legacy_candidates:
    ...
    def _launch_runtime(self, *extra_args: str) -> None:
        ...
        logged_popen(
            ...
            watch_exit=False,
            description="hidden .sst-runtime.exe (pywebview shell)",
        )
