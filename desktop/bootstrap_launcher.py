...
from desktop.bootstrap_payload import (
    ...
    is_github_release_tag_relevant_for_local,
...
class BootstrapLauncher:
    ...
    def _fetch_latest_release(self, *, local_version: str) -> tuple[str | None, str | None]:
        """
        ...
        for item in payload:
            ...
            if not version:
                continue
            if not is_github_release_tag_relevant_for_local(local_version, version):
                continue
    ...
    def _maybe_prompt_update(self, manifest: PayloadManifest) -> None:
        ...
        latest, url = self._fetch_latest_release(local_version=current)
