...
5. The selected install profile is saved locally in:
   - `backend/data/install_profile.txt`
6. Browser opens:
   - `http://127.0.0.1:8765/`
7. OBS overlay URL:
   - `http://127.0.0.1:8765/overlay`
...
7. model directory creation (`backend\data\models`)
...
- Shared install-critical wheels are pinned to versions verified on a clean Windows machine:
  - `numpy==2.2.6`
  - `onnxruntime==1.23.2`
- Runtime caches and temporary files are forced into the project directory:
  - `.cache\pip`
  - `.cache\huggingface`
  - `.cache\torch`
  - `.cache\matplotlib`
...
If no override flag is passed, both scripts reuse the locally saved profile from `backend/data/install_profile.txt`.
...
- All configuration is JSON-only and local-only under `backend/data/`.
- Main live config file: `backend/data/config.json`.
- `backend/data/config.json` and `backend/data/profiles/*.json` are local machine files and should stay out of git.
- Safe committed references live in:
  - `backend/data/config.example.json`
- If `config.json` is missing, the backend auto-generates a default local config on startup using built-in defaults, so first-run startup still works without copying any file manually.
...
- Profiles are stored as JSON files under `backend/data/profiles/`:
  - example: `backend/data/profiles/default.json`
- Dashboard supports:
  - save current config
...
- ASR backend preference is stored locally under `asr` in `backend/data/config.json`:
  - `provider_preference`
  - `prefer_gpu`
...
- Local install destination:
  - `backend/data/models/parakeet-tdt-0.6b-v3/parakeet-tdt-0.6b-v3.nemo`
...
- Runtime behavior:
  ...
  - if the model is missing, the first `Start` in the dashboard now downloads it automatically into `backend/data/models/`
...
- Export files are written locally under `backend/data/exports/`.
...
1. Open `backend/data/config.json`.
2. Set:
...
nvidia-smi
```
- In the current audited environment, the machine GPU is visible to Windows/NVIDIA tools, but the project `.venv` is using:
...
2. Download the official EU multilingual model:
   - `.\.venv\Scripts\python.exe -m backend.install_asr_model --model eu`
3. Optional ASR backend selection in `backend/data/config.json`:
